...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        // 3. Delete all transactions for this batch from TEMP table
        const [deleteResult] = await connection.query<mysql.ResultSetHeader>(
            'DELETE FROM TEMP_TRANSACTIONS WHERE upload_batch_id = ?',
             [batchId]
        );
        ...
        console.error(`Error rejecting transaction batch ${batchId}:`, error);
        if (connection) {
            await connection.rollback();
        }
